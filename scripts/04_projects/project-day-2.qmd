...
There will be some pre-filled project templates in the folder that goes through the whole process for 1 easy and 1 hard question for each dataset (which will stress the specific "skills to practice" for each dataset; primarily, these are focused on tidyverse approaches). These are effectively mini-lessons on their own, but hopefully straightforward enough for a student to follow along on their own. You may want to take a glance at these before class.

You can reiterate to the class that for those who want a bit more structure/support, you will be most familiar with the details for those questions (i.e. they should try to recreate those scripts on their own because you'll have the "answer").

Other things:

-   re: "easier" and "harder" questions, those divisions are mostly based off how much wrangling effort/function wizardry is involved. There any many ways to answer the questions. And obviously, those aren't the only questions you can ask for the datasets, so feel free to come up with more.

-   Some "Hints" are provided for specific code that might not have been covered yet (e.g. splitting strings, certain counting functions) – would encourage students to take a look at them anyway even if they feel really comfortable with R.
:::
:::
...
Today, now that you've been through the entire course, we want to reinforce some of the skills that you've learned by having you apply your knowledge to several existing datasets and have you go through the wrangling, analysis, and visualization process as much as you can on your own. By the end of this lesson, you should have a GitHub repo containing scripts for wrangling and analysis for at least one of these datasets that you can show to others (or at least have as reference for your future self).
...
::: callout-note
Though you should have most of the basic skills you need to wrangle and analyze these datasets, there are some instances where we have specifically provided the code to aid you in certain wrangling tasks: see the "Analysis ideas and hints" callout for each dataset.
:::
