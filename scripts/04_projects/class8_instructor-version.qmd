...
To help with your analyses, we have included an **template document** outlining these steps that you are welcome to use.

## Datasets

The csvs are available under the `data` folder. We have provided links to data that is publicly available for download, as well as some related publications that may give you more information about the data.
...
That said, we have prepared some example research questions and figures for each dataset to get everyone started, so you can decide if you'd rather focus more on coding or science-ing for each dataset. These research questions are split into "easier" and "harder" coding challenges, with "harder" challenges generally involving slightly more wrangling: all datasets have a **pre-filled template** in the `example-analyses` folder that will explain the code used to produce the example figures.
