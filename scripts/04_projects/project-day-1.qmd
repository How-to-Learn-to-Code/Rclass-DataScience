...
## Objectives of Project Management Day 1

-   Understand the importance of reproducible code

-   Be able to describe methods used to follow good computational practices

-   Create a GitHub account and create your first repository
...
knitr::include_graphics("project-day-1_files/thisisfine.gif")
```
...
"An undergrad left their code in `/pine` \[UNC's *temporary* data storage space\] when they went home over the summer and it got deleted, so they had to re-write their code from scratch (which delayed the project as a whole)"
...
knitr::include_graphics("project-day-1_files/charlie-day.gif")
```
...
knitr::include_graphics("project-day-1_files/thumbs_up.gif")
```
:::

### Go on, `git`!

`git` is version control **software** used to record changes to files. [GitHub]{.blue} is a repository hosting **service** that uses `git` to help users host/review code and manage projects. You can use git locally (on your own computer) without ever using GitHub. One of the many benefits of also using GitHub is that it stores your repositories (folders with code) in the cloud. On GitHub, it is free to have public repositories, but a paid account is required if you want several private repositories. As a student (or with a .edu email address) you can get GitHub Pro for free.
...
[Git and GitHub are not:]{.underline}

-   These tools are not Google Drive. If you try to add large files, you'll run into issues.

-   Similar to the first point, these tools are also not a secure place to store data. If you work with sensitive data of any kind, be careful with what is being publicly shared on GitHub. You can always set repositories to private and share with specific collaborators. You should be able to recreate all intermediate data files, plots, results, etc. using your scripts anyways!
...
Let's start by getting the terminal open. In the top left, click View -\> Move Focus To Terminal. It should have opened in the panel that also contains tabs for "Console" and "Background Jobs".
...
Running this command should print your public SSH key to your screen. Copy this to your clipboard, ensuring the entire key is highlighted (it's pretty long!).
...
Now, let's go over to Github and set up the link.
...
2.  `git config --global user.name "<your-github-username>"`
    ...
    -   Keep the quotes around your username, e.g. `git config --global user.name "ramses"`

3.  `git config --global user.email <your-email>`

    -   No quotes this time, e.g. `git config --global user.email ramses@unc.edu`
...
2.  Give it a name, maybe "HTLTC_project"
...
We'll explain the significance of the README and .gitignore steps a bit later. For now, lets go over to our new repo on our profile. To get it onto Longleaf, we can:
...
Below is an example of how we may setup directories, or folders, for our project:
...
-   Stratify files between too many directories <!-- can make this just as hard to find -->
-   Use spaces in file names! <!-- escaping spaces on the command line with git is annoying -->
...
There are three basic commands you need to push changes from your local repository (your computer) to the remote repository (GitHub): `git add`, `git commit`, and `git push`.

![Image modified from: https://dev.to/mollynem/git-github\--workflow-fundamentals-5496](project-day-1_files/git_workflow.png)
...
There is almost a 100% chance you get a git error you do not immediately the meaning of or how to solve it. There is also a nearly 100% chance someone has also run into this issue, and has asked about it on the internet. Often, git will provide an idea of how to fix the issue (e.g. `fatal: The current branch project-update has no upstream branch. To push the current branch and set the remote as upstream, use git push --set-upstream origin project-update)` but if you are still confused, copy and paste the error message into Google!

There are a ton of resources/turotrials out there on using git and GitHub. Here are a few instructors have found helpful. Note that some may cover git actions we did not discuss (such as branching).

-   [Hello World - GitHub Docs](https://docs.github.com/en/get-started/start-your-journey/hello-world)

-   Practicing git commands in a sandbox environment: [learn git branching](https://learngitbranching.js.org), [visualizing git](https://git-school.github.io/visualizing-git/)
