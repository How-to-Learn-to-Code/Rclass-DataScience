...
::: callout-tip
There are two pipe operators: `|>` and `%>%`. They work almost the exact same way. `%>%` is from the `magrittr` package and was the only way to pipe before version R 4.1.0. 

::: {.callout-note title="Placeholders" collapse="true"}

The **Placeholder** operator allows more control over where the `LHS` is placed into the `RHS` of the pipe operator.
