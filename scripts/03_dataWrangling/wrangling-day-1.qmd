...
::: callout-tip
The universe of associated packages are called the `tidyverse`, and it's a hot-topic in the R world. We'll get into specifics of how it differs from what we've been doing so far.
:::
...
Data scientists often want to make many changes to their data at one time. Typically, this means using more than one function at once. However, the way we've been writing our scripts so far would make for some very confusing looking code. For example, let's use `dplyr` functions to perform two operations on our dataset of scientists: filter for those born after 1900 and then arrange them by date of birth.
...
filtered_data <- filter(data, as.Date(date_of_birth) > as.Date("1900-01-01"))
...
This approach does the same thing but without creating an intermediate variable. It's more compact but can start to get confusing if we add more functions.

```{r}
arranged_data <- arrange(filter(data, as.Date(date_of_birth) > as.Date("1900-01-01")), date_of_birth)
```

## Using pipes to clean up the code
The pipe operator (`%>%`) from the `magrittr` package, which is incorporated into `dplyr`, allows us to pass the result of one function directly into the next. This makes the code easier to read and maintain.
...
The pipe version simplifies how we look at transformations. We can see each step as a part of a continuous workflow, modifying the dataset in a logical sequence without needing to store intermediate results. This helps keep our code both clean and readable.
::: 
...
In our case, we're looking at the proportion of cell types in each sample. \*Whole refers to \_\_\_ and fraction refers to \_\_\_\*. Proportions, by nature, sum up to 1. In this case, checking that the values in each row sum to 1 is one way to confirm that we have what we're expecting. Lets go ahead and check that:
