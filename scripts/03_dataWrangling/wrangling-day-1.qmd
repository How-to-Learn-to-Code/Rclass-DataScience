...
We can also use `filter` to keep rows based on multiple conditions. For example, to filter our data frame, `df`, by scientists who went to "Cambridge" and were born after 1900, we would run:

```{r}
df |> 
  filter(school == "Cambridge" & date_of_birth > "1900-01-01")
```
...
You can use `desc()` on a column inside of `arrange()` to re-order the data frame based on that column in descending (big-to-small) order. For example:

```{r}
df |> 
  arrange(desc(date_of_birth))
```
...
Alternatively, if you want to the keep other columns when filtering for unique rows, you can use the `.keep_all = TRUE option`. For example:
...
df |> 
  mutate(rate_out_of_10 = rate_out_of_10 + 1)
```

`select` allows you to focus on the variables you’re interested in and rapidly zoom in on a useful subset using operations based on the names of the variables:

You can select columns by name:
```{r}
df |> 
  select(name, date_of_birth, school)
```

You can select columns between `name` and `date_of_birth`:
...
Finally, you can even select all columns that are characters:
...
```{r}
df |> 
  select(full_name = name, dob = date_of_birth)
```

You can select all columns except those from `name` to `date_of_birth`:

```{r}
df |> 
  select(-c(name:date_of_birth))
...
```{r}
df |> 
  rename(full_name = name, dob = date_of_birth)
```

`relocate` allows you to move variables around. You might want to collect related variables together or move important variables to the front. By default `relocate()` moves variables to the front:

```{r}
df |> 
  relocate(date_of_birth)
```
...
```{r}
df |> 
  relocate(date_of_birth, .after = name)
```
...
```{r}
df |> 
  group_by(school, working_region)
```
...
```{r}
df |> 
  summarize(mean_rate = mean(rate_out_of_10))
```
...
`ungroup` allows you to remove grouping from a data frame without using `summarize()`
