...
::: callout-tip
There are two pipe operators: `|>` and `%>%`. They work almost the exact same way. `%>%` is from the `magrittr` package and was the only way to pipe before version R 4.1.0. `%>%` is more lenient in its capabilities.
...
`%>%` uses `.` to represent the the `LHS` in the `RHS`. In addition to this, you may use `.` multiple times on the `RHS`
...
`|>` uses `_` to represent the `LHS` in the `RHS`. However, the `_` placeholder must only be used once and the argument must be named
...
```{r}
#| echo: false
cat("Error in sum(2, \"_\") : \n  pipe placeholder can only be used as\n   a named argument")
```
...
#| echo: false
cat("Error in head : 
  The pipe operator requires a function \n  call as RHS")
```
...
#| echo: false
cat("Error in { : 
  function '{' not supported in RHS call \n  of a pipe")
```
