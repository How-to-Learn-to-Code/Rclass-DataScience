...
-   Our data is measures of proportions of cell types across samples
-   There control samples made of mostly pure cell types (`fractions`) and experimental samples, composed of a range of cell types
-   The experimental samples are from wild-type/knock-out and treatment/control
-   There are `.csv` files for both the cell type proportion data (`cellProportions.csv`) and the sample phenotypes (`cellPhenotypes.csv`)


And in general, there are a few things to think about when wrangling/exploring data:
...
Our data fits the `tidy` style we introduced last class, since each row is a sample (observation) and each column is a different cell type (variable).
...
If you continue with programming, there will be points when you need to share your code with some or return to code you wrote months to years ago. When that happens, you'll appreciate leaving easy-to-understand scripts!
:::
...
Data with many observations and samples is often formatted as wide or long. Our current data is in a wide format, which has a single row for each sample and a column for each variable.

When wide data is pivoted into a long format columns are condensed together. It's easiest to understand how pivoting works by seeing it done:
...
We want to make a column of proportions values and columns specifying cell types.
