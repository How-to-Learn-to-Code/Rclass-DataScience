...
Welcome to the first day of data visualization! Hopefully today will feel fun and rewarding as you will learn how to create beautiful plots to explore data, communicate insights, and maybe even include in a publication! We will show you how to use the two most popular methods to create plots: base R and `ggplot`. Let's dive in! 📊

## Objectives of Data Visualization: Class 1

-   Build a basic plot using base R functions

-   Build a basic plot using the `ggplot` package

-   Change design elements of plots using base R and `ggplot`
...
```{r}
variants <- read.table("DataVizDay1_files/variants_from_assembly.bed", sep="\t", quote='', stringsAsFactors=TRUE,header=FALSE)
