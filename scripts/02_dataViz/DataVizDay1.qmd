...
For this lesson, we will be using a .bed file of genetic variants from <https://marianattestad.com/blog>. We will need to read in this data using the `read.csv()` function, then rename the columns with the `name()` function:

```{r}
variants <- read.csv("DataVizDay1_files/variants_from_assembly.bed", sep="\t", quote='', stringsAsFactors=TRUE,header=FALSE)
